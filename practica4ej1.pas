...
{ Retornar la cantidad de códigos que existen en el árbol que son menores que un valor que se recibe como parámetro }
  ...
  begin
		if(a<>nil)then begin
			...
			else CantidadDeCodigosMenores:= CantidadDeCodigosMenores(a^.HI, cod);
		end else CantidadDeCodigosMenores:= 0;
  end;
  ...
  begin
		if(a<>nil)then begin
			if(a^.dato.codigo>codigo1) then
				...
				else ObtenerMontoTotalEntreDosCodigos:= ObtenerMontoTotalEntreDosCodigos(a^.HI, codigo1,codigo2)
			else ObtenerMontoTotalEntreDosCodigos:= ObtenerMontoTotalEntreDosCodigos(a^.HD, codigo1,codigo2);
		end else ObtenerMontoTotalEntreDosCodigos:= 0;
  end;
...
begin
  ...
  then writeln ('No hay codigos entre ', codigo1, ' y ', codigo2)
         ...
         writeln;
         writeln ('El monto total entre el codigo', codigo1, ' y el codigo : ', codigo2, ' es: ', montoTotal:1:2); 
         writeln;
       end;
  writeln;
  writeln ('-----------------------------------------------');
