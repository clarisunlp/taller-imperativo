...
procedure InformarExistenciaNombreSocio (a:arbol);
  ...
  function ExisteSocio(a:arbol;nom:cadena15):boolean;
  begin
    if(a<>nil)then
      if (a^.dato.nombre = nom) then ExisteSocio:=true
      else ExisteSocio:= ExisteSocio(a^.HI,nom) or ExisteSocio(a^.HD,nom)
    else ExisteSocio:=false;
  end;
...
var
  ...
  ok:boolean;
begin
  writeln('ingrese el nombre del socio a buscar'); readln(nombre);
  ok:=ExisteSocio(a,nombre);
  if ok then writeln('el nombre ' , nombre , ' existe en la estructura')
  else writeln('el nombre ' , nombre , ' no existe en la estructura');
end;
